...
// ===== LIQUIDITY CALCULATION FUNCTION =====
async function calculateLiquidityRatio() {
  // Check if required elements and contracts are available
  if (!contracts.simpleSwap || !elements.liquidityAmountA) {
    if (elements.liquidityAmountB) elements.liquidityAmountB.value = "";
    return;
  }
  ...
  // Clear Token B field if Token A is empty
  ...
  try {
    ...
    // Check if this is the first liquidity addition (no reserves)
    if (reserveA.isZero() && reserveB.isZero()) {
      // For first liquidity addition, user can set any ratio
      // We'll show a message or leave Token B empty for user to decide
      if (elements.liquidityAmountB) elements.liquidityAmountB.value = "";

      // Optional: Show a message that this is initial liquidity
      console.log("First liquidity addition - you can set any ratio");
      return;
    }

    // Calculate required Token B amount to maintain current pool ratio
    const amountAParsed = ethers.utils.parseEther(amountA);
    const requiredTokenB = amountAParsed.mul(reserveB).div(reserveA);

    // Format and display the calculated amount
    const formattedAmountB = parseFloat(
      ethers.utils.formatEther(requiredTokenB)
    ).toFixed(6);

    if (elements.liquidityAmountB) {
      elements.liquidityAmountB.value = formattedAmountB;
    }

    console.log(`For ${amountA} Token A, you need ${formattedAmountB} Token B`);
  } catch (error) {
...
// ===== REVERSE LIQUIDITY CALCULATION FUNCTION =====
async function calculateLiquidityRatioReverse() {
  // Check if required elements and contracts are available
  if (!contracts.simpleSwap || !elements.liquidityAmountB) {
    if (elements.liquidityAmountA) elements.liquidityAmountA.value = "";
    return;
  }
  ...
  // Clear Token A field if Token B is empty
  ...
  try {
    ...
    // Calculate required Token A amount to maintain current pool ratio
    const amountBParsed = ethers.utils.parseEther(amountB);
    const requiredTokenA = amountBParsed.mul(reserveA).div(reserveB);

    // Format and display the calculated amount
    const formattedAmountA = parseFloat(
      ethers.utils.formatEther(requiredTokenA)
    ).toFixed(6);

    if (elements.liquidityAmountA) {
      elements.liquidityAmountA.value = formattedAmountA;
    }

    console.log(`For ${amountB} Token B, you need ${formattedAmountA} Token A`);
  } catch (error) {
    console.error("Error calculating reverse liquidity ratio:", error);
    if (elements.liquidityAmountA) elements.liquidityAmountA.value = "";
  }
}

// ===== MODIFIED SETUP EVENTS FUNCTION =====
function setupEvents() {
  console.log("Setting up event listeners...");

  // ... existing event listeners ...

  // Liquidity calculation events
  if (elements.liquidityAmountA) {
    elements.liquidityAmountA.addEventListener(
      "input",
  ...
  if (elements.liquidityAmountB) {
    elements.liquidityAmountB.addEventListener(
      "input",
      calculateLiquidityRatioReverse
    );
  }
...
// ===== MODIFIED ADD LIQUIDITY FUNCTION =====
async function addLiquidity() {
  if (!userAddress || !contracts.simpleSwap) {
    showNotification("Connect your wallet first", "error");
    return;
  ...
  try {
    ...
    // Get current reserves to adjust slippage tolerance
    const [reserveA, reserveB] = await contracts.simpleSwap.getReserves(
      CONTRACT_ADDRESSES.TOKEN_A,
      CONTRACT_ADDRESSES.TOKEN_B
    );
    ...
    // If pool is empty, use tighter slippage
    if (reserveA.isZero() && reserveB.isZero()) {
      amountAMin = amountADesired.mul(95).div(100); // 5% slippage for new pools
      amountBMin = amountBDesired.mul(95).div(100);
    } else {
      // For existing pools, use more permissive slippage since we calculated exact ratio
      amountAMin = amountADesired.mul(90).div(100); // 10% slippage
      amountBMin = amountBDesired.mul(90).div(100);
    }
